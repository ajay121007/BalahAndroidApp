...
/**
 * Created by Thakur on 4/3/2018.
 */

public class AuctionDetailsById {

    private static final String TAG = AuctionDetailsById.class.getSimpleName();
    public static ArrayList<AuctionBean> list;

    public void AuctionDetailsById(Context context, String url) {
        list = new ArrayList<>();
        new AuctionDetailsById.ExecuteApi(context).execute(url);
            ...
            if (s != null) {
                try {
                    ...
                    if (jsonArray.length() >= 1) {
                        ...
                        EventBus.getDefault().post(new Event(Constants.AUCTIONSETAILBYID, ""));
                    } else {
