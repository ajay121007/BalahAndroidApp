...
public class CustomActivity extends BasePaymentActivity implements ITransactionListener {

    @BindView(R.id.rv_payment_mode)
    RecyclerView rvPaymentMode;
    ...
    private ServiceConnection serviceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            providerBinder.addTransactionListener(CustomActivity.this);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        intent = getIntent();
        if (intent != null && intent.getExtras().containsKey(Constants.GRANDTOTAL)) {
            BasePaymentActivity.totalAmount = intent.getExtras().getString(Constants.GRANDTOTAL);
            tvTotalAmount.setText("SAR " + BasePaymentActivity.totalAmount);
        }

        tvTitle.setText("Pay By Visa");
        paymentModeList.add("Hyper Pay Visa");
        paymentModeList.add("Hyper Pay Master");
        paymentModeList.add("Cash On Delivery");

        layoutManager = new LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false);
        rvPaymentMode.setLayoutManager(layoutManager);
        adapter = new PaymentModeAdapter(this, paymentModeList, new PaymentModeAdapter.AdapterClick() {
            @Override
            public void Onclick(int position) {
                switch (position) {
                    ...
                    case 1:
                        tvTitle.setText("Pay By Master");
                        break;
                }
            }
        });
        rvPaymentMode.setAdapter(adapter);
    ...
    private void initViews() {
        cardHolderEditText = (EditText) findViewById(R.id.holder_edit_text);
        cardHolderEditText.setText(Constants.Config.CARD_HOLDER_NAME);

        cardNumberEditText = (EditText) findViewById(R.id.number_edit_text);
        cardNumberEditText.setText(Constants.Config.CARD_NUMBER);

        cardExpiryMonthEditText = (EditText) findViewById(R.id.expiry_month_edit_text);
        cardExpiryMonthEditText.setText(Constants.Config.CARD_EXPIRY_MONTH);

        cardExpiryYearEditText = (EditText) findViewById(R.id.expiry_year_edit_text);
        cardExpiryYearEditText.setText(Constants.Config.CARD_EXPIRY_YEAR);

        cardCvvEditText = (EditText) findViewById(R.id.cvv_edit_text);
    ...
    private void pay(String checkoutId) {
        try {
            ...
            Log.d("PaymentException", e.getMessage().toString());
    ...
    private PaymentParams createPaymentParams(String checkoutId) throws PaymentException {
        ...
        PaymentParams params = new CardPaymentParams(
                checkoutId,
    ...
    public void transactionCompleted(Transaction transaction) {
        ...
        if (transaction.getTransactionType() == TransactionType.ASYNC) {
            ...
            requestPaymentStatus(checkoutId);
    ...
    public void transactionFailed(Transaction transaction, PaymentError paymentError) {
        hideProgressDialo();
        Log.d("ErrorTransaction", paymentError.getErrorMessage() + "   d " + paymentError.getErrorInfo());
        showErrorDialog(paymentError);
    }
